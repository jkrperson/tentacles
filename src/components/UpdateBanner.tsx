...
export function UpdateBanner() {
  ...
  const visible = status.status === 'available' || status.status === 'downloading' || status.status === 'downloaded'
  if (!visible) return null

  const expiresAt = snoozedUntil(status.version)
  if (status.status === 'downloaded' && expiresAt > Date.now()) return null
  ...
  const message = isDownloaded
    ...
    : status.status === 'downloading'
      ? ` is downloading… ${typeof status.progress === 'number' ? `${status.progress}%` : ''}`
